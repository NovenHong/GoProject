...
var (
	USERNAME3 string = "admin"
	PASSWORD3 string = "game123456" //game123456
	NETWORK3  string = "tcp"
	SERVER3   string = "120.132.31.222"
	PORT3     int    = 3306
...
func isExistChargeRoleKeepData(chargeRoleKeepData ChargeRoleKeepData) (id int64) {
	querySql := fmt.Sprintf(`SELECT id FROM gc_charge_role_keep2 WHERE date_time = %d AND channel_id = %d AND game_id = %d AND server_id = %d LIMIT 1`,
		chargeRoleKeepData.DateTime,
		chargeRoleKeepData.ChannelId,
...
func saveChargeRoleKeepData(chargeRoleKeepData ChargeRoleKeepData) {
	...
	if chargeRoleKeepData.Id > 0 {
		querySql = `UPDATE gc_charge_role_keep2 SET 
		...
		querySql = `insert INTO gc_charge_role_keep2
		(date,date_time,channel_id,game_id,server_id,charge_num,keep_num_1,keep_num_3,keep_num_7,keep_num_14,keep_num_20,keep_num_30,keep_num_60,keep_num_90)
		values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
		_, err = DB.Exec(
			querySql,
...
func isExistEffectiveRoleKeepData(effectiveRoleKeepData EffectiveRoleKeepData) (id int64) {
	querySql := fmt.Sprintf(`SELECT id FROM gc_effective_role_keep2 WHERE date_time = %d AND channel_id = %d AND game_id = %d AND server_id = %d LIMIT 1`,
		effectiveRoleKeepData.DateTime,
		effectiveRoleKeepData.ChannelId,
...
func saveEffectiveRoleKeepData(effectiveRoleKeepData EffectiveRoleKeepData) {
	...
	if effectiveRoleKeepData.Id > 0 {
		querySql = `UPDATE gc_effective_role_keep2 SET 
		...
		querySql = `insert INTO gc_effective_role_keep2
		(date,date_time,channel_id,game_id,server_id,effective_num,keep_num_1,keep_num_3,keep_num_7,keep_num_14,keep_num_30,keep_num_60,keep_num_90)
		values(?,?,?,?,?,?,?,?,?,?,?,?,?)`
		_, err = DB.Exec(
			querySql,
...
func isExistCreateRoleKeepData(createRoleKeepData CreateRoleKeepData) (id int64) {
	querySql := fmt.Sprintf(`SELECT id FROM gc_create_role_keep2 WHERE date_time = %d AND channel_id = %d AND game_id = %d AND server_id = %d LIMIT 1`,
		createRoleKeepData.DateTime,
		createRoleKeepData.ChannelId,
...
func saveCreateRoleKeepData(createRoleKeepData CreateRoleKeepData) {
	...
	if createRoleKeepData.Id > 0 {
		querySql = `UPDATE gc_create_role_keep2 SET 
		...
		querySql = `insert INTO gc_create_role_keep2
		(date,date_time,channel_id,game_id,server_id,create_num,keep_num_1,keep_num_3,keep_num_7,keep_num_14,keep_num_30,keep_num_60,keep_num_90)
		values(?,?,?,?,?,?,?,?,?,?,?,?,?)`
		_, err = DB.Exec(
			querySql,
...
func getChannelDatas() (channelDatas []ChannelData) {
	querySql := "SELECT channel_id FROM gc_channel WHERE type = 1"

	rows, err := DB3.Query(querySql)

	if err != nil {
		panic(fmt.Sprintf("Get channel data error, Error:%s", err))
	}
